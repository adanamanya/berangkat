...
export default class MovieList extends Component {
  ...
  fetchSchedule(theater) {
    ...
    queryRef.get()
      ...
      sortedMovies = _.sortBy(theaterMovies, 'title');
    ...
    <Container style={{marginTop: -210}} >
      {this.state.loading? <Spinner color={this.state.spinnerColor} /> :
        <ScrollView>
          ...
          <Image style={{ width: width * 0.88, height: height * 0.32, marginTop: -30}} source={{uri: movies.img}}/>
          <Text style={styles.cardText}>{movies.title}</Text> 
          <Text style={styles.cardText2}>{movies.theater}</Text> 
          </View>
          </HideWithKeyboard>
            ) 
             }}
