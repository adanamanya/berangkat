...
assignCity() {
  const userId = firebez.auth().currentUser.uid;
  const userCity = firebez.database().ref('/users/' + userId).once('value').then((snapshot) => {
    const data = snapshot.val();
    console.log(data.city);
    if (data.city == undefined) {
      this.setState({ citeh: 'Jakarta'});
    }
    else {
      this.setState({ citeh: data.city });
    }
    this.getmoviesnp();
    this.getNews();
    this.getnearhotels();
 ...
 async getmoviesnp() {
  console.log(this.state.citeh);
    ...
    const queryRef = moviesnpref.where("city", "==", this.state.citeh);
    let allmovies = [];
    let sortedmovies = [];
    queryRef.get()
  .then(snapshot => {
 ...
 async getnearhotels() {
    ...
    const queryRef = hotelref.where("location", "==", "jakarta");
    let allhotels = [];
    let sortedhotel = [];
  ...
  //alert(this.state.city.toString().substr(0,this.state.city.toString().indexOf(' ')));
    ...
    return (
      <View style={styles.movieContainer}>
        <Text style={styles.title}>Now Playing in {this.state.citeh}</Text>
        ...
        <TouchableOpacity onPress={ () => {
           rnfirebase.analytics().logEvent('search_movie_press');
            this.movies()}}>
          <Text style={[styles.hotelSubtitle, { textDecorationLine: 'underline' }]}>Search for movies</Text>
        </TouchableOpacity>
        <Carousel
          ...
          <View>
            ...
            <Button transparent onPress={ 
              () => {rnfirebase.analytics().logEvent('viewprofile');
                Actions.profiledefault({type: 'reset'})
              }} >
